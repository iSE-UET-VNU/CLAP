...
from FileManager import get_spc_log_file_path, join_path, EXPERIMENT_RESULT_FOLDER
...
def mutiple_bugs_ranking(system_name, system_dir, spectrum_expressions, filtering_coverage_rate):
    ...
    #spc_log_file_path = SPCsManager.find_SPCs(system_dir, filtering_coverage_rate)
    #spc_log_file_path = get_spc_log_file_path(system_dir, filtering_coverage_rate)
    #SlicingManager.do_slice(spc_log_file_path, filtering_coverage_rate, "")
    suspicious_stms_list = get_suspicious_statement(system_dir, filtering_coverage_rate)
    buggy_statements = get_multiple_buggy_statements(system_name, system_dir)
    ...
    for aggregation_type in aggregations:
        for normalization_type in normalizations:
            ...
            for sbfl_expression in range(0, len(spectrum_expressions)):
                ranking_results, space = ranking_multiple_bugs(buggy_statements, system_dir,
