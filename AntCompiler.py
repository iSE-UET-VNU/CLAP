from FileManager import get_plugin_path, get_file_name, get_src_dir, get_compiled_classes_dir
...
def compile_classes(variant_dir):
    ...
    output_log = execute_shell_command(f'/bin/sh {PLUGIN_PATH}', extra_args=[
        ...
        {"-out": get_compiled_classes_dir(variant_dir)},
    ])
    if not output_log.find("BUILD SUCCESSFUL"):
        logger.fatal(f"{output_log}\n\nFailed to compile source code [{variant_dir}]")
    return output_log
